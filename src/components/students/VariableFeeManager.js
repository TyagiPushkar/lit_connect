...
import {
    ...
    Button, TextField, Snackbar, Table, TableBody,
    TableCell, TableContainer, TableHead, TableRow, Paper, Autocomplete
} from '@mui/material';
...
const VariableFeeManager = () => {
    ...
    const fetchVariableFees = async (studentId) => {
        try {
            const res = await axios.get(`https://namami-infotech.com/LIT/src/fees/variable.php?student_id=${studentId}`);
            if (res.data.success) {
    ...
    const handleAddFee = async () => {
        ...
        try {
            const res = await axios.post('https://namami-infotech.com/LIT/src/fees/variable.php', {
    ...
    const handleSampleDownload = () => {
        ...
        let csvContent = "data:text/csv;charset=utf-8," + [header, ...sample].map(e => e.join(",")).join("\n");
    ...
    const handleFileUpload = async () => {
        ...
        try {
            const res = await axios.post('https://namami-infotech.com/LIT/src/fees/variable.php', formData, {
    ...
    return (
        <div style={{ padding: 0 }}>
            ...
            <Autocomplete
                ...
                renderInput={(params) => <TextField {...params} label="Select Student" sx={{ width: "500px" }} />}
                isOptionEqualToValue={(option, value) => option.StudentID === value.StudentID}
            />

<Button
    variant="outlined"
    sx={{ color: '#CC7A00', borderColor: '#CC7A00', '&:hover': { borderColor: '#b56700', color: '#b56700' } }}
    onClick={handleSampleDownload}
>
    Download Sample
</Button>
                <input type="file" accept=".xls,.xlsx,.csv" onChange={(e) => setUploadFile(e.target.files[0])} />
                <Button
    variant="contained"
    sx={{ backgroundColor: '#CC7A00', '&:hover': { backgroundColor: '#b56700' } }}
...
>
    Add Variable Fee
                    ...
                    <TableContainer component={Paper}>
                        ...
                        <TableHead>
    <TableRow>
        <TableCell sx={{ backgroundColor: '#CC7A00', color: 'white' }}>Particular</TableCell>
                                    <TableCell sx={{ backgroundColor: '#CC7A00', color: 'white' }}>Amount</TableCell>
                                    <TableCell sx={{ backgroundColor: '#CC7A00', color: 'white' }}>Status</TableCell>
    </TableRow>
</TableHead>
                                ...
                                {variableFees.length > 0 ? (
                                    variableFees.map((fee, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>{Number(fee.Paid) === 1 ? 'Paid' : 'Not Paid'}</TableCell>
