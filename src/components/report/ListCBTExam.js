...
function ListCBTExam({ setView }) {
    ...
    const fetchRecords = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('http://139.5.190.143/LIT/src/report/list_cbt_exam.php', {
    ...
    const handleSave = async (rowId) => {
        try {
            const { data } = await axios.post('http://139.5.190.143/LIT/src/report/edit_cbt.php', {
                Id: rowId,
