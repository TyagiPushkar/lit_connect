...
const FinalizeAdmissionView = () => {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      const hostel = state?.hostel || localStorage.getItem("hostel");
      // console.log("Course:",course)
      if (!studentId || !course || !hostel) {
        alert("Student ID, Course or Hostel not found. Please go back and try again.");
        setLoading(false);
        return;
      }
      ...
      const fetchFeeStructure = async () => {
        try {
          ...
          if (res.data.success && Array.isArray(res.data.data)) {
            const matched = res.data.data.filter((item) => item.course.trim().toLowerCase() === course.trim().toLowerCase());
