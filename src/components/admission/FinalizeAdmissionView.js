...
const course = state?.course || localStorage.getItem("course");
    ...
    const fetchFeeStructure = async () => {
      try {
        const res = await axios.get("https://namami-infotech.com/LIT/src/fees/get_fee_structure.php");
        console.log("API response:", res.data);

        if (res.data.success && Array.isArray(res.data.data)) {
          const matched = res.data.data.filter((item) => item.course === course);
          console.log("Filtered by course:", matched);
  ...
  if (!formDataList.length) {
    return (
      <Box sx={{ mt: 5, textAlign: "center" }}>
        <Typography variant="h6" color="error">No fee structure available for this course.</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: 2, background: "#fff", pb: 10 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        <Typography variant="h5" fontWeight={700} color="#CC7A00">
